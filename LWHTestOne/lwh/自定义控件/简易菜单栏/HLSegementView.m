...
- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles textColor:(UIColor *)textColor selectTextColor:(UIColor *)selectTextColor lineColor:(UIColor *)lineColor{
...
- (void)calculatorWidths{
    ...
    for (NSString *title in self.titles) {
        ...
        NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithObject:[UIFont systemFontOfSize:TextFont+ 2] forKey:NSFontAttributeName];
...
- (void)setUpSegmentView{
    ...
    for (int i = 0; i < self.titles.count; i++) {
        ...
        label.font = [UIFont systemFontOfSize:TextFont - 1];
...
//    NSLog(@"获取当前角标%ld",i);
    ...
    if ([self.delegate respondsToSelector:@selector(hl_didSelect:withIndex:)]) {
        [self.delegate hl_didSelect:self withIndex:i];
    }
...
- (void)selectLabel:(UILabel *)label
{
    for (UILabel *labelView in self.titleLabels) {
        ...
        labelView.font = [UIFont systemFontOfSize:TextFont - 1];
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    self.underLine.y = label.height - underLineH - 4;
...
- (UIScrollView *)titleScrollView{
    if (!_titleScrollView) {
        ...
        _titleScrollView.backgroundColor = self.backgroundColor;
...
- (UIView *)underLine
{
    if (_underLine == nil) {
        ...
        underLineView.backgroundColor = _underLineColor?_underLineColor:self.lineColor;
