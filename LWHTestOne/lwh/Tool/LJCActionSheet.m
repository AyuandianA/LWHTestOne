...
+ (LJCActionSheet *)actionSheetWithTitle:(NSString *)title confirms:(NSArray *)confirms cancel:(NSString *)cancel style:(LJCActionSheetStyle)style click:(ClickBlock)click{
    ...
    return [[self alloc] initWithTitle:title confirms:confirms cancel:cancel style:style click:click];
}


#pragma mark - 初始化控件
- (instancetype)initWithTitle:(NSString *)title confirms:(NSArray *)confirms cancel:(NSString *)cancel style:(LJCActionSheetStyle)style
{
    self = [super init];
    if (self) {
        self.title = title;
...
#pragma mark - 初始化控件
- (instancetype)initWithTitle:(NSString *)title confirms:(NSArray *)confirms cancel:(NSString *)cancel style:(LJCActionSheetStyle)style click:(ClickBlock)click
{
    self = [super init];
    if (self) {
        self.title = title;
...
- (void)createSubViews
{
    ...
    if (self.title && self.title.length) {
        UIFont *font = [UIFont systemFontOfSize:XJSize_13];
        ...
        CGSize titleSize = [self.title boundingRectWithSize:maxSize
                                                 ...
                                                 attributes:attributes
                                                    context:nil].size;
        titleHeight = titleSize.height + XJSize_40;
...
//         titleLabel.backgroundColor = [UIColor orangeColor];
        ...
        titleLabel.font = [UIFont systemFontOfSize:XJSize_13];
        titleLabel.frame = CGRectMake(10, 0, SCREEN_WIDTH-20, titleHeight);
    ...
    for (int i=0; i<self.confirms.count; i++) {
        ...
        confirm.titleLabel.font = [UIFont systemFontOfSize:XJSize_17];
        
        [confirm setTitleColor:[UIColor darkTextColor] forState:UIControlStateNormal];
            
        if (self.style == LJCActionSheetStyleDestructive) {
            [confirm setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        }else if(self.style == LJCActionSheetStyleCancel){
            confirm.titleLabel.font = [UIFont boldSystemFontOfSize:TextFont];
        }
    ...
    separator.frame = CGRectMake(0, titleHeight + confirmHeight, SCREEN_WIDTH, XJSize_5);
    [self addSubview:separator];
    separatorHeight = XJSize_5;
    ...
    cancel.titleLabel.font = [UIFont systemFontOfSize:XJSize_17];
    ...
    CGFloat ActionSheetHeight = titleHeight + confirmHeight + separatorHeight + cancelHeight + SafeAreaH;
...
- (void)button:(UIButton *)button clickAtIndex:(NSUInteger)index
{
    if (self.clickBlock) {
        ...
        if ([self.delegate respondsToSelector:@selector(clickAction:atIndex:sheetTitle:)]) {
            [self animationHideShadowView];
            [self animationHideActionSheet];
            NSInteger index = [self.confirms indexOfObject:button.titleLabel.text];
            [self.delegate clickAction:self atIndex:index sheetTitle:button.titleLabel.text];
        }
    }
}
